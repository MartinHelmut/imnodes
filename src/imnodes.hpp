...
using ImNodesCol = int;              // -> enum ImNodesCol_
using ImNodesStyleVar = int;         // -> enum ImNodesStyleVar_
using ImNodesStyleFlags = int;       // -> enum ImNodesStyleFlags_
using ImNodesPinShape = int;         // -> enum ImNodesPinShape_
using ImNodesAttributeFlags = int;   // -> enum ImNodesAttributeFlags_
using ImNodesMiniMapLocation = int;  // -> enum ImNodesMiniMapLocation_
...
struct ImNodesStyle {
  ...
  // The radius from the pin's center position inside which it is detected as being hovered
  // over.
  ...
  // Set these mid-frame using Push/PopColorStyle. You can index this color array with a
  // ImNodesCol value.
  std::array<unsigned int, ImNodesCol_COUNT> Colors;
...
// BeginNodeEditor/EndNodeEditor pair)
//
...
using ImNodesMiniMapNodeHoveringCallback = void (*)(int, void*);
#endif

#ifndef ImNodesMiniMapNodeHoveringCallbackUserData
using ImNodesMiniMapNodeHoveringCallbackUserData = void*;
#endif
...
void MiniMap(float minimap_size_fraction = 0.2F,
    ImNodesMiniMapLocation location = ImNodesMiniMapLocation_TopLeft,
    ImNodesMiniMapNodeHoveringCallback node_hovering_callback = nullptr,
    ImNodesMiniMapNodeHoveringCallbackUserData node_hovering_callback_data = nullptr);
...
void SetNodeDraggable(int node_id, bool draggable);
...
bool IsLinkCreated(
    int* started_at_attribute_id, int* ended_at_attribute_id, bool* created_from_snap = nullptr);
...
const char* SaveEditorStateToIniString(
    const ImNodesEditorContext* editor, size_t* data_size = nullptr);
